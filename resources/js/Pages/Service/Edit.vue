...
const form = useForm({
    ...
    price: Number(props.service.price) || null,
    currency: props.service.currency || 'USD',
    weight_limit: props.service.weight_limit || '',
    destinations: props.service.destinations || [],
...
const submit = () => {
    form.put(route('services.update', props.service.id), {
        onSuccess: () => {
            toast.add({ severity: 'success', summary: 'Success', detail: 'Service updated successfully', life: 3000 });
        },
        onError: () => {
        ...
        <div class="rounded-sm border border-stroke bg-white px-5 pt-6 pb-6 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5">
            <form @submit.prevent="submit" class="flex flex-col gap-6">
                ...
                <div class="flex justify-end mt-4">
                    <Button label="Update Service" type="submit" icon="pi pi-check" :loading="form.processing" />
                </div>
            </form>
        </div>
    </TailAdminLayout>
