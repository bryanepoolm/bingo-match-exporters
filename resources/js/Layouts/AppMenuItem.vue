...
<template>
    <li :class="{ 'layout-root-menuitem': root, 'active-menuitem': isActiveMenu }">
        ...
        <Link v-if="item.to && !item.items && item.visible !== false" :href="route(item.to)" @click="itemClick($event, item)" :class="[item.class, { 'active-route': checkActiveRoute(item) }]" tabindex="0">
            <i :class="item.icon" class="layout-menuitem-icon"></i>
            <span class="layout-menuitem-text">{{ item.label }}</span>
            <i class="pi pi-fw pi-angle-down layout-submenu-toggler" v-if="item.items"></i>
        </Link>
        <Transition name="layout-submenu">
            <ul v-if="item.items && item.visible !== false" v-show="isActiveMenu">
